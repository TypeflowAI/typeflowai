  ...
  return (
    <>
      {triggers.map((triggerEventClassId, idx) => (
        <div className="mt-2" key={idx}>
          <div className="flex items-center gap-2">
            <p className="mr-2 w-10 text-right text-sm text-slate-800">{idx === 0 ? "When" : "or"}</p>
            <Select
              value={triggerEventClassId}
              onValueChange={(eventClassId) => setTriggerEvent(idx, eventClassId)}>
              <SelectTrigger className="xs:w-[180px] xs:text-base w-full p-1.5 text-xs text-slate-800">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {actionClasses.map((actionClass) => (
                  <SelectItem
                    key={actionClass.id}
                    className="xs:text-base px-0.5 py-1 text-xs text-slate-800"
      ...
      <div className="ml-4 w-fit p-3">
        <Button
          variant="secondary"
          className="xs:text-base w-fit text-xs"
          onClick={() => {
            addTriggerEvent();
          }}>
