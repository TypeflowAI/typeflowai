...
function HighlightQuery({ text, query }: { text: string; query: string }) {
  return (
    <Highlighter
      highlightClassName="underline bg-transparent text-brand"
      searchWords={[query]}
      autoEscape={true}
  ...
  return (
    <li
      ...
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="group-aria-selected:text-brand text-sm font-medium text-slate-900">
        ...
        <div
          ...
          {hierarchy.map((item, itemIndex, items) => (
            <Fragment key={itemIndex}>
              ...
              <span className={itemIndex === items.length - 1 ? "sr-only" : "mx-2 text-slate-300"}>/</span>
            </Fragment>
          ))}
        </div>
      )}
    ...
    return (
      <div className="p-6 text-center">
        ...
        <p className="mt-2 text-xs text-slate-700">
          Nothing found for{" "}
          <strong className="break-words font-semibold text-slate-900">&lsquo;{query}&rsquo;</strong>. Please
          try again.
        </p>
      </div>
    );
      ...
      <Dialog onClose={setOpen} className={clsx("fixed inset-0 z-50", className)}>
        ...
        <div className="fixed inset-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-20 md:py-32 lg:px-8 lg:py-[15vh]">
          <Transition.Child
            ...
            <Dialog.Panel className="ring-slate-900/7.5 mx-auto transform-gpu overflow-hidden rounded-lg bg-slate-50 shadow-xl ring-1 sm:max-w-xl">
...
export function Search() {
  ...
  return (
    <div className="hidden lg:block lg:max-w-md lg:flex-auto">
      <button
        type="button"
        className="ui-not-focus-visible:outline-none hidden h-8 w-full items-center gap-2 rounded-lg bg-white pl-2 pr-3 text-sm text-slate-500 ring-1 ring-slate-900/10 transition hover:ring-slate-900/20 lg:flex"
        {...buttonProps}>
        <SearchIcon className="h-5 w-5 stroke-current" />
        Find something...
        <kbd className="text-2xs ml-auto text-slate-400">
...
export function MobileSearch() {
  ...
  return (
    <div className="contents lg:hidden">
      <button
        type="button"
        className="ui-not-focus-visible:outline-none flex h-6 w-6 items-center justify-center rounded-md transition hover:bg-slate-900/5 lg:hidden"
