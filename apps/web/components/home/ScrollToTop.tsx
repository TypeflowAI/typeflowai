...
const ScrollToTopButton = () => {
  ...
  return (
    <div className="fixed bottom-20 right-10 z-50" style={{ display: visible ? "inline" : "none" }}>
      <Button
        className="flex w-12 items-center justify-center bg-slate-900/10 px-1 py-2 hover:bg-slate-900/20 hover:opacity-100"
        onClick={scrollToTop}>
        <ArrowUpIcon className="h-6 w-6 text-slate-900" />
      </Button>
    </div>
  );
};
