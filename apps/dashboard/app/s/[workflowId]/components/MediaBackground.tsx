...
export const MediaBackground: React.FC<MediaBackgroundProps> = ({
  ...
  // get the background from either the workflow or the product styling
  const background = useMemo(() => {
    ...
    if (product.styling.allowStyleOverwrite) {
      ...
      return workflow.styling.background;
    }

    return product.styling.background;
  }, [product.styling.allowStyleOverwrite, product.styling.background, workflow.styling]);
            ...
            <div className={`${baseClasses} ${loadedClass} bg-cover bg-center`}>
              ...
              {authorDetailsForUnsplash.authorName && (
                <div className="absolute bottom-4 right-6 z-10 ml-auto hidden w-max text-xs text-slate-400 md:block">
                  <span>Photo by </span>
                  <Link
                    href={authorDetailsForUnsplash.authorURL + "?utm_source=typeflowai&utm_medium=referral"}
                    target="_blank"
                  ...
                  <span> on </span>
                  <Link
                    href="https://unsplash.com/?utm_source=typeflowai&utm_medium=referral"
                    target="_blank"
                    className="hover:underline">
                    Unsplash
                  </Link>
