...
import { GetActiveInactiveWorkflowsAction } from "@/app/(app)/environments/[environmentId]/(actionsAndAttributes)/attributes/actions";
...
export default function AttributeActivityTab({ attributeClass }: EventActivityTabProps) {
  ...
  useEffect(() => {
    ...
    async function getWorkflows() {
      try {
        setLoading(true);
        const activeInactive = await GetActiveInactiveWorkflowsAction(attributeClass.id);
        setActiveWorkflows(activeInactive.activeWorkflows);
        setInactiveWorkflows(activeInactive.inactiveWorkflows);
      } catch (err) {
        setError(err);
      } finally {
        setLoading(false);
