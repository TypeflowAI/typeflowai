...
import Placement from "@/app/(app)/environments/[environmentId]/workflows/[workflowId]/edit/components/Placement";
...
import { TWorkflow, TWorkflowProductOverwrites } from "@typeflowai/types/workflows";
import { Label } from "@typeflowai/ui/Label";
import { Switch } from "@typeflowai/ui/Switch";

interface WorkflowPlacementCardProps {
  localWorkflow: TWorkflow;
  setLocalWorkflow: (workflow: TWorkflow) => void;
  environmentId: string;
}

export default function WorkflowPlacementCard({
  localWorkflow,
  setLocalWorkflow,
  environmentId,
}: WorkflowPlacementCardProps) {
  const [open, setOpen] = useState(false);

  const { productOverwrites } = localWorkflow ?? {};
  const { placement, clickOutsideClose, darkOverlay } = productOverwrites ?? {};

  const setProductOverwrites = (productOverwrites: TWorkflowProductOverwrites) => {
    setLocalWorkflow({ ...localWorkflow, productOverwrites });
  };
    ...
    <Collapsible.Root
      ...
      <Collapsible.CollapsibleTrigger
        ...
        <div className="inline-flex px-4 py-4">
          ...
          <div>
            <p className="font-semibold text-slate-800">Workflow Placement</p>
            <p className="mt-1 text-sm text-slate-500">Overwrite the global placement of the workflow</p>
          </div>
        </div>
      </Collapsible.CollapsibleTrigger>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center space-x-1">
              ...
              <Label htmlFor="workflowDeadline" className="cursor-pointer">
                <div className="ml-2">
                  ...
                  <p className="text-xs font-normal text-slate-500">Change the placement of this workflow.</p>
                </div>
              </Label>
            </div>
            {placement && (
            ...
            <div>
              <p className="text-xs text-slate-500">
                To keep the placement over all workflows consistent, you can{" "}
