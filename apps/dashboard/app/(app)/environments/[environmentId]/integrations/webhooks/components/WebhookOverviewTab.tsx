...
const getWorkflowNamesForWebhook = (webhook: TWebhook, allWorkflows: TWorkflow[]): string[] => {
  if (webhook.workflowIds.length === 0) {
    return allWorkflows.map((workflow) => workflow.name);
  } else {
    return webhook.workflowIds.map((workflowId) => {
...
export default function WebhookOverviewTab({ webhook, workflows }: ActivityTabProps) {
  return (
    <div className="grid grid-cols-3 pb-2">
      <div className="col-span-2 space-y-4 pr-6">
        ...
        <div>
          <Label className="text-slate-500">Workflows</Label>

          {getWorkflowNamesForWebhook(webhook, workflows).map((workflowName, index) => (
