...
import { TWebhookInput } from "@typeflowai/types/webhooks";
...
export default function AddWebhookModal({ environmentId, workflows, open, setOpen }: AddWebhookModalProps) {
  ...
  const [selectedAllWorkflows, setSelectedAllWorkflows] = useState(false);
  ...
  const handleSelectedWorkflowChange = (workflowId: string) => {
    setSelectedWorkflows((prevSelectedWorkflows: string[]) =>
      prevSelectedWorkflows.includes(workflowId)
        ? prevSelectedWorkflows.filter((id) => id !== workflowId)
    ...
    if (!isSubmitting) {
      try {
        ...
        if (!selectedAllWorkflows && selectedWorkflows.length === 0) {
          throw new Error("Please select at least one workflow");
        }
          ...
          <div className="flex w-full items-center justify-between p-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="text-sm text-slate-500">Send workflow response data to a custom endpoint</div>
              </div>
            </div>
          </div>
        </div>
