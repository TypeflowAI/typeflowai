...
export default function TargetingCard({
  ...
  const handleCloneSegment = async () => {
    ...
    try {
      const clonedSegment = await cloneBasicSegmentAction(segment.id, localWorkflow.id);
  ...
  const handleLoadNewSegment = async (workflowId: string, segmentId: string) => {
    const updatedWorkflow = await loadNewBasicSegmentAction(workflowId, segmentId);
  ...
  const handleResetAllFilters = async () => {
    try {
      return await resetBasicSegmentFiltersAction(localWorkflow.id);
    } catch (err) {
              ...
              {isAdvancedSegment(segment?.filters ?? []) ? (
                <div>
                  <SegmentTitle
                    ...
                    isPrivate={localWorkflow.segment?.isPrivate}
                  />
                        ...
                        {!segment?.isPrivate ? (
                          ...
                          <div className="mb-4">
                            <p className="text-sm font-semibold text-slate-800">
                              Send workflow to audience who match...
                            </p>
                          </div>
                        )}
                      </div>
