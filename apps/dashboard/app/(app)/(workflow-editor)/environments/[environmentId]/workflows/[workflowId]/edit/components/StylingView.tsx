...
import { TBaseStyling } from "@typeflowai/types/styling";
...
export const StylingView = ({
  ...
  const stylingDefaults: TBaseStyling = useMemo(() => {
    ...
    const isOverwriteEnabled = localWorkflow.styling?.overwriteThemeStyling ?? false;

    if (isOverwriteEnabled) {
      const { overwriteThemeStyling, ...baseWorkflowStyles } = localWorkflow.styling ?? {};
      stylingDefaults = baseWorkflowStyles;
    } else {
  ...
  }, [localWorkflow.styling, product.styling]);

  const form = useForm<TWorkflowStyling>({
    defaultValues: {
  ...
  }, [watchedValues, setLocalWorkflow]);
