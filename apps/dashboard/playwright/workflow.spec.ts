...
test.describe("Workflow Create & Submit Response", async () => {
  ...
  test("Create Workflow with Custom Actions", async ({ page }) => {
    const { name, email, password } = users.workflow[1];
  ...
  test("Submit Workflow Response", async ({ page }) => {
    ...
    for (let i = 0; i < workflows.createAndSubmit.singleSelectQuestion.options.length; i++) {
      await expect(page.getByText(workflows.createAndSubmit.singleSelectQuestion.options[i])).toBeVisible();
    }
    await expect(page.getByText("Other")).toBeVisible();
    await expect(page.getByRole("button", { name: "Next" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Back" })).toBeVisible();
    await page.getByText(workflows.createAndSubmit.singleSelectQuestion.options[0]).click();
    await page.getByRole("button", { name: "Next" }).click();
    ...
    for (let i = 0; i < workflows.createAndSubmit.multiSelectQuestion.options.length; i++) {
      await expect(page.getByText(workflows.createAndSubmit.multiSelectQuestion.options[i])).toBeVisible();
    }
    await expect(page.getByRole("button", { name: "Next" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Back" })).toBeVisible();
    await page.getByText(workflows.createAndSubmit.multiSelectQuestion.options[0]).click();
    await page.getByText(workflows.createAndSubmit.multiSelectQuestion.options[1]).click();
    await page.getByRole("button", { name: "Next" }).click();
    ...
    await expect(page.getByText(workflows.createAndSubmit.ratingQuestion.lowLabel)).toBeVisible();
    await expect(page.getByText(workflows.createAndSubmit.ratingQuestion.highLabel)).toBeVisible();
    ...
    await expect(page.getByText(workflows.createAndSubmit.npsQuestion.lowLabel)).toBeVisible();
    await expect(page.getByText(workflows.createAndSubmit.npsQuestion.highLabel)).toBeVisible();
    await expect(page.getByRole("button", { name: "Next" })).not.toBeVisible();
    await expect(page.getByRole("button", { name: "Back" })).toBeVisible();
