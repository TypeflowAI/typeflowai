...
export const WelcomeCard = ({
  ...
  const calculateTimeToComplete = () => {
    let idx = calculateElementIdx(workflow, 0);
    if (idx === 0.5) {
      idx = 1;
    }
    const timeInSeconds = (workflow.questions.length / idx) * 15; //15 seconds per question.
  ...
  const timeToFinish = workflow.welcomeCard.timeToFinish;
  const showResponseCount = workflow.welcomeCard.showResponseCount;
