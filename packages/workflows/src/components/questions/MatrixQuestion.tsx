...
import type { TI18nString, TWorkflowMatrixQuestion } from "@typeflowai/types/workflows";
                  ...
                  <tr className={`${rowIndex % 2 === 0 ? "bg-input-bg" : ""}`}>
                    ...
                    {question.columns.map((column, columnIndex) => (
                      <td
                        key={columnIndex}
                        tabIndex={0}
                        className={`outline-brand px-4 py-2 text-gray-800 ${
                          columnIndex === question.columns.length - 1 ? "rounded-r-custom" : ""
                        }`}
                        onClick={() =>
                          handleSelect(
                            getLocalizedValue(column, languageCode),
