...
export const addCustomThemeToDom = ({ styling }: { styling: TProductStyling | TWorkflowStyling }) => {
  // Check if the style element already exists
  let styleElement = document.getElementById("typeflowai__css__custom");
  ...
  if (styling.cardShadowColor?.light) {
    ...
    appendCssVariable("workflow-shadow-color", mixColor(styling.cardShadowColor.light, "#ffffff", 0.4));
  }
  ...
  appendCssVariable("workflow-background-color", styling.cardBackgroundColor?.light);
  appendCssVariable("workflow-border-color", styling.cardBorderColor?.light);
