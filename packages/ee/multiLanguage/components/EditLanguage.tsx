...
import {
  ...
  getWorkflowsUsingGivenLanguageAction,
...
export default function EditLanguage({ product, environmentId }: EditLanguageProps) {
  ...
  const handleDeleteLanguage = async (languageId: string) => {
    try {
      const workflowsUsingLanguage = await getWorkflowsUsingGivenLanguageAction(product.id, languageId);

      if (workflowsUsingLanguage.length > 0) {
        const workflowList = workflowsUsingLanguage.map((workflowName) => `• ${workflowName}`).join("\n");
        setConfirmationModal({
          isOpen: true,
          languageId,
          text: `You cannot remove this language since it’s still used in these workflows:\n\n${workflowList}\n\nPlease remove the language from these workflows in order to remove it from the product.`,
  ...
  return (
    <TooltipProvider delayDuration={80}>
      <Tooltip>
        ...
        <TooltipContent>
          The alias is an alternate name to identify the language in link workflows and the SDK (optional)
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
