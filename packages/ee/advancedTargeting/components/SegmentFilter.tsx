...
} from "@typeflowai/lib/segment/utils";
...
type TAttributeSegmentFilterProps = TSegmentFilterProps & {
  ...
  updateValueInLocalWorkflow: (filterId: string, newValue: TSegmentFilterValue) => void;
};
...
const AttributeSegmentFilter = ({
  ...
  const updateOperatorInLocalWorkflow = (filterId: string, newOperator: TAttributeOperator) => {
    const updatedSegment = structuredClone(segment);
    if (updatedSegment.filters) {
  ...
  const updateAttributeClassNameInLocalWorkflow = (filterId: string, newAttributeClassName: string) => {
    const updatedSegment = structuredClone(segment);
    if (updatedSegment.filters) {
  ...
  const checkValueAndUpdate = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateValueInLocalWorkflow(resource.id, value);
    ...
    if (ARITHMETIC_OPERATORS.includes(operator as TArithmeticOperator)) {
      ...
      if (isNumber.success) {
        setValueError("");
        updateValueInLocalWorkflow(resource.id, parseInt(value, 10));
      } else {
        setValueError("Value must be a number");
        updateValueInLocalWorkflow(resource.id, value);
      }
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <Select
        ...
        onValueChange={(value) => {
          updateAttributeClassNameInLocalWorkflow(resource.id, value);
        }}
        disabled={viewOnly}>
        <SelectTrigger
      ...
      <Select
        ...
        onValueChange={(operator: TAttributeOperator) => {
          updateOperatorInLocalWorkflow(resource.id, operator);
        }}
        disabled={viewOnly}>
...
const PersonSegmentFilter = ({
  ...
  const updatePersonIdentifierInLocalWorkflow = (filterId: string, newPersonIdentifier: string) => {
    const updatedSegment = structuredClone(segment);
    if (updatedSegment.filters) {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <Select
        ...
        onValueChange={(value) => {
          updatePersonIdentifierInLocalWorkflow(resource.id, value);
        }}
        disabled={viewOnly}>
        <SelectTrigger
          className="flex w-auto items-center justify-center whitespace-nowrap bg-white capitalize"
...
const ActionSegmentFilter = ({
  ...
  const updateActionMetricInLocalWorkflow = (filterId: string, newMetric: TActionMetric) => {
    const updatedSegment = structuredClone(segment);
    if (updatedSegment.filters) {
    ...
    <div className="flex items-center gap-2 text-sm">
      ...
      <Select
        ...
        onValueChange={(value: TActionMetric) => {
          updateActionMetricInLocalWorkflow(resource.id, value);
        }}
        disabled={viewOnly}>
        <SelectTrigger
          className="flex w-auto items-center justify-center whitespace-nowrap bg-white capitalize"
