...
import { DatabaseError, OperationNotAllowedError } from "@typeflowai/types/errors";
  ...
  try {
    ...
    if (!actionClass) {
      throw new OperationNotAllowedError(
        `${name} action unknown. Please add this action in TypeflowAI first in order to use it in your code.`
      );
    }
