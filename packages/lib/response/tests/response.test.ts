...
describe("Tests for getResponses service", () => {
  describe("Happy Path", () => {
    it("Fetches first 10 responses for a given workflow ID", async () => {
      const response = await getResponses(mockWorkflowId, 1, 10);
      expect(response).toEqual([expectedResponseWithoutPerson]);
    });
  });
  ...
  describe("Tests for getResponses service with filters", () => {
    describe("Happy Path", () => {
      it("Fetches all responses for a given workflow ID with basic filters", async () => {
        ...
        const response = await getResponses(mockWorkflowId, 1, undefined, { finished: true });

        expect(expectedWhereClause).toEqual({ workflowId: mockWorkflowId, ...whereClause });
        expect(response).toEqual(getFilteredMockResponses({ finished: true }));
      });

      it("Fetches all responses for a given workflow ID with complex filters", async () => {
        ...
        const response = await getResponses(mockWorkflowId, 1, undefined, criteria);
...
describe("Tests for getWorkflowSummary service", () => {
  describe("Happy Path", () => {
  ...
  describe("Sad Path", () => {
    testInputValidation(getWorkflowSummary, 1);
...
describe("Tests for getResponseDownloadUrl service", () => {
  describe("Happy Path", () => {
    it("Returns a download URL for the csv response file", async () => {
      ...
      const url = await getResponseDownloadUrl(mockWorkflowId, "csv");
    ...
    it("Returns a download URL for the xlsx response file", async () => {
      ...
      const url = await getResponseDownloadUrl(mockWorkflowId, "xlsx", { finished: true });
      const fileExtension = url.split(".").pop();
  ...
  describe("Sad Path", () => {
    testInputValidation(getResponseDownloadUrl, mockWorkflowId, 123);
    ...
    it("Throws error if response file is of different format than expected", async () => {
      ...
      const url = await getResponseDownloadUrl(mockWorkflowId, "csv", { finished: true });
      const fileExtension = url.split(".").pop();
      expect(fileExtension).not.toEqual("xlsx");
    });

    it("Throws DatabaseError on PrismaClientKnownRequestError, when the getResponseCountByWorkflowId fails", async () => {
      const mockErrorMessage = "Mock error message";
      const errToThrow = new Prisma.PrismaClientKnownRequestError(mockErrorMessage, {
        code: "P2002",
        clientVersion: "0.0.1",
      ...
      await expect(getResponseDownloadUrl(mockWorkflowId, "csv")).rejects.toThrow(DatabaseError);
    });
    ...
    it("Throws a generic Error for unexpected problems", async () => {
      ...
      // error from getWorkflow
      prisma.workflow.findUnique.mockRejectedValue(new Error(mockErrorMessage));

      await expect(getResponseDownloadUrl(mockWorkflowId, "xlsx")).rejects.toThrow(Error);
    });
  });
});
