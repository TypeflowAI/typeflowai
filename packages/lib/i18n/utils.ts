...
export const extractLanguageCodes = (workflowLanguages: TWorkflowLanguage[]): string[] => {
  if (!workflowLanguages) return [];
  return workflowLanguages.map((workflowLanguage) =>
    workflowLanguage.default ? "default" : workflowLanguage.language.code
  );
};

export const getEnabledLanguages = (workflowLanguages: TWorkflowLanguage[]) => {
  return workflowLanguages.filter((workflowLanguage) => workflowLanguage.enabled);
};
...
// LGEGACY
// Helper function to maintain backwards compatibility for old workflow objects before Multi Language
...
export const getLanguageCode = (workflowLanguages: TWorkflowLanguage[], languageCode: string | null) => {
