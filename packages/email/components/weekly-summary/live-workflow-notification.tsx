...
import type { TWorkflowStatus } from "@typeflowai/types/workflows";
import { EmailButton } from "../general/email-button";
import { renderEmailResponseValue } from "../workflow/response-finished-email";
...
const convertWorkflowStatus = (status: TWorkflowStatus): string => {
  const statusMap = {
...
export const LiveWorkflowNotification = ({ environmentId, workflows }: LiveWorkflowNotificationProps) => {
  const createWorkflowFields = (workflowResponses: TWeeklySummaryWorkflowResponseData[]) => {
    ...
    const workflowFields: JSX.Element[] = [];
